...
class GrammarBackedRootNode extends GrammarBackedNode {
  ...
  updateNodeTypeIds(nodeTypeMap) {
    if (typeof nodeTypeMap === "string") nodeTypeMap = new TreeNode_1.default(nodeTypeMap)
    if (nodeTypeMap instanceof TreeNode_1.default) nodeTypeMap = nodeTypeMap.toObject()
    const renames = []
  ...
  getPrettified() {
    ...
    clone._firstWordSort(
      nodeTypeOrder.split(" "),
      isCondensed ? TreeUtils_1.default._makeGraphSortFunction(node => node.getWord(1), node => node.get(GrammarConstants.extends)) : undefined
    )
...
class AbstractCellError extends AbstractTreeError {
  ...
  _getWordSuggestion() {
    return TreeUtils_1.default.didYouMean(
...
class GrammarEnumTestNode extends AbstractGrammarWordTestNode {
  ...
  getOptions() {
    if (!this._map) this._map = TreeUtils_1.default.arrayToMap(this.getWordsFrom(1))
    return this._map
  }
}
class AbstractExtendibleTreeNode extends TreeNode_1.default {
  ...
  _getChildrenByNodeConstructorInExtended(constructor) {
    return this._getAncestorsArray()
      .map(node => node.getChildrenByNodeConstructor(constructor))
      .flat()
  }
  _getExtendedParent() {
...
class AbstractGrammarDefinitionNode extends AbstractExtendibleTreeNode {
 ...
 ${className}
  ...
  _getCatchAllNodeTypeDefinition() {
    ...
    else return this.getParent()._getCatchAllNodeTypeDefinition()
  }
...
class GrammarProgram extends AbstractGrammarDefinitionNode {
  ...
  getNodeTypeDefinitionByFirstWordPath(firstWordPath) {
    ...
    for (let index = 0; index < parts.length; index++) {
      ...
      if (!def) def = subject._getCatchAllNodeTypeDefinition()
      subject = def
    }
  ...
  ${rootClassMethods.join("\n")}
      ...
      getGrammarProgramRoot() {
        if (!this._cachedGrammarProgramRoot)
          this._cachedGrammarProgramRoot = jtree.GrammarProgram.newFromCondensed(\`${TreeUtils_1.default.escapeBackTicks(
