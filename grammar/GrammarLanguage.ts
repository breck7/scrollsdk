...
abstract class GrammarBackedNode extends TreeNode {
	...
	private _getAutocompleteResultsForFirstWord(partialWord: string) {
		...
		return keywords
			.map(keyword => {
				...
				return {
					text: keyword,
					displayText: keyword + (description ? TN_WORD_BREAK_SYMBOL + description : "")
				}
			})
			.filter(i => i)
	}
	...
	toAutoCompleteTable() {
		return new TreeNode(
			<any>this._getAllAutoCompleteWords().map(result => {
				result.suggestions = <any>result.suggestions.map((node: any) => node.text).join(TN_WORD_BREAK_SYMBOL)
				return result
			})
		).asTable
	}
	...
	getParserUsage(filepath = "") {
		...
		handGrammarProgram.validConcreteAndAbstractParserDefinitions.forEach((def: AbstractParserDefinitionParser) => {
			...
			usage.appendLine([def.parserIdFromDefinition, "line-id", "parser", requiredCellTypeIds.join(TN_WORD_BREAK_SYMBOL)].join(TN_WORD_BREAK_SYMBOL))
		})
		this.topDownArray.forEach((node: GrammarBackedNode, lineNumber: number) => {
			const stats = usage.getNode(node.parserId)
			stats.appendLine([filepath + "-" + lineNumber, node.words.join(TN_WORD_BREAK_SYMBOL)].join(TN_WORD_BREAK_SYMBOL))
		})
		return usage
	}
	...
	toDefinitionLineNumberTree() {
		return this.topDownArray.map((child: GrammarBackedNode) => child.definition.lineNumber + TN_WORD_BREAK_SYMBOL + child.indentation + child.cellDefinitionLineNumbers.join(TN_WORD_BREAK_SYMBOL)).join("\n")
	}
	...
	get lineCellTypes() {
		return this.parsedCells.map(slot => slot.cellTypeId).join(TN_WORD_BREAK_SYMBOL)
	}
	...
	getLineCellPreludeTypes() {
		return this.parsedCells
			...
			.join(TN_WORD_BREAK_SYMBOL)
	}

	getLineHighlightScopes(defaultScope = "source") {
		return this.parsedCells.map(slot => slot.highlightScope || defaultScope).join(TN_WORD_BREAK_SYMBOL)
	}
...
abstract class AbstractGrammarBackedCell<T> {
	...
	synthesizeCell(seed = Date.now()): string {
		...
		if (enumOptions) return Utils.getRandomString(1, enumOptions.split(TN_WORD_BREAK_SYMBOL))
...
${options.toString(1)}`
	...
	protected _getErrorContext() {
		return this.getNode().getLine().split(TN_WORD_BREAK_SYMBOL)[0] // todo: WordBreakSymbol
	}
...
class GrammarAnyCell extends AbstractGrammarBackedCell<string> {
	...
	_synthesizeCell() {
		...
		if (examples) return Utils.getRandomString(1, examples.split(TN_WORD_BREAK_SYMBOL))
...
class GrammarReservedWordsTestParser extends AbstractGrammarWordTestParser {
	...
	isValid(str: string) {
		if (!this._set) this._set = new Set(this.content.split(TN_WORD_BREAK_SYMBOL))
...
class EnumFromCellTypesTestParser extends AbstractGrammarWordTestParser {
	_getEnumFromCellTypes(programRootNode: GrammarBackedNode): treeNotationTypes.stringMap {
		...
		const enumGroup = cellTypeIds.join(TN_WORD_BREAK_SYMBOL)
...
abstract class AbstractCellParser {
	...
	getRequiredCellTypeIds(): treeNotationTypes.cellTypeId[] {
		if (!this._requiredCellTypeIds) {
			...
			this._requiredCellTypeIds = parameters ? parameters.split(TN_WORD_BREAK_SYMBOL) : []
		}
...
${properties.join("\n")}
	...
	private _getMyCellTypeDefs() {
		...
		return requiredCells.split(TN_WORD_BREAK_SYMBOL).map(cellTypeId => {
...
${cells.toString(1)}`
	...
	get cruxPath() {
		...
		return (parentPath ? parentPath + TN_WORD_BREAK_SYMBOL : "") + this.cruxIfAny
	}
	...
	synthesizeNode(nodeCount = 1, indentCount = -1, parsersAlreadySynthesized: string[] = [], seed = Date.now()) {
		...
		while (nodeCount) {
			...
			if (line) lines.push(TN_WORD_BREAK_SYMBOL.repeat(indentCount >= 0 ? indentCount : 0) + line)
...
${testCode}`
	...
	get parserFamilyTree() {
		...
		Object.values(this.validConcreteAndAbstractParserDefinitions).forEach(node => tree.touchNode(node.ancestorParserIdsArray.join(TN_WORD_BREAK_SYMBOL)))
		return tree
	}
	...
	private get fileExtensions(): string {
		return this.rootParserDefinition.get(GrammarConstants.extensions) ? this.rootParserDefinition.get(GrammarConstants.extensions).split(TN_WORD_BREAK_SYMBOL).join(",") : this.extensionName
	}
