...
testParticles.getZeroedBoard = (equal: any) => {
  ...
  testCases.forEach(test => {
    equal(new Particle(SweeperCraftGame.getZeroedBoard(test.rows, test.cols)).toString(), new Particle(test.expected).toString(), "deep equal okay")
  })
}
...
testParticles.genRandomBoard = (equal: any) => {
  ...
  const deepEqual = new Particle(board).toString() === new Particle(board2).toString()
