...
---\n${this._childrenToYaml(0).join("\n")}`
  ...
  _toOutline(nodeFn) {
    const growBranch = (outlineParticle, last, lastStates, nodeFn, callback) => {
      let lastStatesCopy = lastStates.slice(0)
      const node = outlineParticle.node
  ...
  static fromGridJson(str) {
    ...
    const cellDelimiter = language.wordBreakSymbol
    const nodeDelimiter = language.nodeBreakSymbol
    return new Particle(lines.map(line => line.join(cellDelimiter)).join(nodeDelimiter))
  }
  static fromSsv(str) {
    return this.fromDelimited(str, " ", '"')
  }
...
module.exports = { Particle, ExtendibleParticle, AbstractExtendibleParticle, TreeEvents, TreeWord }
