...
testParticles.forEach = equal => {
  ...
  var result = ""
  // Act
  value.forEach(function (particle) {
  ...
  // Act
  value2
    .filter(particle => particle.firstWord !== "hello")
    .forEach(particle => {
      const property = particle.firstWord
...
testParticles.every = equal => {
  // Arrange/Act/Assert
  equal(
    new Particle(`a 2\nb 2\nc 2`).every(particle => particle.getWord(1) === "2"),
    true
  )
}
...
b`)
  const a = test.getParticle("a")
  // Act
  a.replaceParticle(str => str.replace("a", "z"))
  // Assert
  equal(
    test.asString,
    `z
...
testParticles.getIndentation = equal => {
  ...
  equal(testStrings.webpageTrimmed, particle.topDownArray.map(line => line.indentation + line.getLine()).join("\n"))
}
...
testParticles.multiply = equal => {
  ...
  // Non blanks
  const four = new Particle(`o
 o
 o
o
 o
 o`)
  const five = new Particle(`o
o
o
o
o
...
316.1`
  ...
  // Act
  program.particleAt(0).replaceParticle(str => str.replace("+", "-"))
  // Assert
...
testParticles.getStackString = equal => {
  const particle = new Particle(`Thing
 color
  blue
  green`)
  // Act/assert
...
testParticles.getGraphLoop = equal => {
  // Arrange
  const particle = new Particle(`Thing Animal
 color
Animal Thing
 dna`)
  // Act/Assert
...
Animal Thing
 ...
 dna`
  ...
  // Arrange
  const particle2 = new Particle(`Thing
 color
Animal Thing
 dna
Monkey Mammal
...
testParticles.expandedShouldAppendNonMaps = equal => {
  ...
  // Arrange
  const particle = new Particle(`constructors
 example
 example foobar`)
  // Act/Assert
...
testParticles.getCustomIndex = equal => {
  // todo: we need to work on extend so its more straightforward
  // Arrange
  const particle = new Particle(`coke
 id 123
 related 456
pepsi
 type soda
...
testParticles.copyTo = equal => {
  // Arrange
  const value = new Particle(`chart
 title Hello
chart2
 title 2`)
...
testParticles.copyToRegression = equal => {
  // Arrange
  const particle = new Particle(`>something
 class SomeClass
 css
  red
  green
 ...
 @blue
 >div`
  const migrateParticle = particle => {
...
testParticles.parseParticle = equal => {
  ...
  const particle = new iHateTypeScriptSometimes(`foo bar
 foo bar
  particle bar
sub
 leaf`)
...
ohayo
 good
  ...
  // Arrange
  const reg = new Particle(`a
 b
  c
d
 e`)
  // Act/Assert
  const result = reg.topDownArray.map(particle => particle.lineNumber).join(" ")
...
q quantity`)
  ...
  remapped.forEach(particle => particle.remap(expandMapObj))
  const expected = remapped.clone()
  expected.forEach(particle => particle.remap(contractMap))
  // Assert
...
bob
 age 10`)
  ...
  equal(b.previous.firstWord, "bob")
  equal(b.previous.next.firstWord, "john")
  equal(b.next.firstWord, "susy")
  equal(c.next.firstWord, "score")
...
testParticles.sort = equal => {
  ...
  // Act
  particle.sort((a, b) => (b.firstWord < a.firstWord ? 1 : b.firstWord === a.firstWord ? 0 : -1))
  // Assert
...
|Lala 2018|11/02/2018 - 11/03/2018|San Fran|http://www.blah.com|foo|`
  ...
  const table = particle.toMarkdownTableAdvanced(["title", "date", "location", "website"], (value, row, col) => (row ? value : Utils.ucfirst(value)))
  // Assert
...
testParticles.setContentWithSubparticlesRegression = equal => {
  ...
  // Act
  hello.setContentWithSubparticles(`brave
 new world`)
...
testParticles.toStringMethod = equal => {
  ...
  // Arrange
  const particle2 = new Particle("z-index 0")
  // Act
  particle2["z-index"] = 0
  // Assert
...
testParticles.windowsReturnChars = equal => {
  // Arrange
  const particle = new Particle(`one
\r
\rtwo
\r
\r
...
testParticles.traverse = equal => {
  // Arrange
  const traversal = new Particle(`0
 01
 02
  020
  021
...
2`)
  // Act
  const preOrder = traversal.topDownArray.map(particle => particle.getLine()).join(" ")
  ...
  // Arrange
  const wikipediaBinaryTree = new Particle(`f
 b
  a
  d
   c
 ...
 g
  i
   h`)
  // Act
  const wikipreorder = wikipediaBinaryTree.topDownArray.map(particle => particle.getLine()).join("")
...
b
 a
c`).getFiltered(particle => particle.firstWord === "a").length,
    3
  )
}
...
testParticles.getFrom = equal => {
  // Arrange
  const particle = new Particle(`name
...
testParticles.toOutline = equal => {
  // Arrange
  const particle = new Particle(`hello
 world`)
  // Act/assert
  equal(
    particle.asOutline,
...
`
  ...
  equal(
    particle.toMappedOutline(particle => "o"),
    `└o
 └o
`
  )
