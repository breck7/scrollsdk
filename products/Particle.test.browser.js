...
testParticles.forEach = equal => {
  ...
  // Act
  value2
    .filter(particle => particle.cue !== "hello")
    .forEach(particle => {
      const property = particle.cue
...
testParticles.every = equal => {
  // Arrange/Act/Assert
  equal(
    new Particle(`a 2\nb 2\nc 2`).every(particle => particle.getAtom(1) === "2"),
    true
  )
}
...
testParticles.simpleParticleLanguage = equal => {
  ...
  class AdditionParticleParser extends Particle {
    ...
    getNumbers() {
      return this.getAtomsFrom(1).map(atom => parseFloat(atom))
    }
...
testParticles.sort = equal => {
  ...
  // Act
  particle.sort((a, b) => (b.cue < a.cue ? 1 : b.cue === a.cue ? 0 : -1))
  // Assert
...
b
 a
c`).getFiltered(particle => particle.cue === "a").length,
    3
  )
}
...
testParticles.macros = equal => {
  ...
  program.addTransformer((line, subparticles) => {
