...
class AbstractParticleComponentParser extends ParserBackedNode {
...
${new stumpParser(this.toStumpCode()).compile()}
 ...
 bern${Particle.nest(css, 2)}`)
  ...
  renderAndGetRenderReport(stumpNode, index) {
    ...
    const childResults = this._getChildParticleComponents().map((child, index) => child.renderAndGetRenderReport(stumpNodeForChildren, index))
    if (isUpdateOp) {
