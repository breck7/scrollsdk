...
class CommandLineApp {
    ...
    build(commandName, argument) {
        ...
        if (commandName)
            return builder[commandName](argument);
        else
            return builder._help(filePath);
    }
    combine(grammarName) {
    ...
    distribute(combinedFilePath) {
        if (!combinedFilePath)
            throw new Error(`No combinedFilePath provided`);
        ...
        return masterFile.split("#file").map((file) => {
            ...
            if (firstLine.getFirstWord() !== "#file")
                return undefined;
    ...
    _initFile(path, initialString = "") {
        if (!fs.existsSync(path))
            this._write(path, initialString);
    }
    _write(path, content) {
...
${grammars.toTable()}`;
    ...
    _getGrammarPathByGrammarNameOrThrow(grammarName) {
        ...
        if (!node)
            throw new Error(`No registered grammar named '${grammarName}'. Registered grammars are ${this._getRegisteredGrammarNames().join(",")}`);
    ...
    _history(grammarName) {
        ...
        const files = data
            ...
            if (!filepath || !filepath.includes("."))
                return false;
            if (["check", "run", "", "compile"].includes(command))
                return true;
        })
    ...
    async run(programPathOrGrammarName) {
        if (programPathOrGrammarName.includes("."))
            return this._run(programPathOrGrammarName);
        return Promise.all(this._history(programPathOrGrammarName).map(file => this._run(file)));
    }
    runSync(programPathOrGrammarName) {
        if (programPathOrGrammarName.includes("."))
            return this._runSync(programPathOrGrammarName);
    ...
    static async main() {
        ...
        if (app[action]) {
            ...
            print(`Unknown command '${action}'. Type 'tree help' to see available commands.`);
    }
}
if (!module.parent)
    CommandLineApp.main();
