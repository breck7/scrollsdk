...
2\tThe answer, my friend, is...\t\"\"\"Two\"\"\"`;
    ...
    testTree.constructorTests = equal => {
        ...
        equal(tree2
            .getNode("foobar")
            .getNode("one")
            .getContent(), "1", "Value should be 1");
 ...
 000000111222222`;
        ...
        tests.split("\n").forEach((testLine, lineIndex) => {
            ...
            testLine.split("").forEach((char, charIndex) => {
                if (char !== " ")
                    equal(node.getWordIndexAtCharacterIndex(charIndex), parseInt(char), `Character is '${char}'`);
            });
        });
        // Arrange
...
other`;
        ...
        // Act
        tree6.forEach((node) => {
            if (!node.getFirstWord().startsWith("p"))
                return true;
            node.setContent("President");
            node.delete("class");
        });
        // Assert
        equal(tree6.toString(), `presidents President
other`);
    };
...
yo hey`;
    ...
    testTree.forEach = equal => {
        ...
        // Act
        value2
            .filter((node) => node.getFirstWord() !== "hello")
            .forEach((node) => {
            const property = node.getFirstWord();
  ...
  >bam
   ...
   ham
    ...
    testTree.fromDelimited = equal => {
        ...
        // Arrange
        const b = TreeNode.fromDelimited(`name,score
...
b`);
    ...
    testTree.fromSsv = equal => {
        ...
        equal(fixedCol
            .nodeAt(0)
            .getNode("comment")
            .getContent(), testStrings.ssvFixedColumnComment1);
        equal(fixedCol
            .nodeAt(1)
            .getNode("comment")
            .getContent(), testStrings.ssvFixedColumnComment2);
    ...
    testTree.getIndentation = equal => {
        ...
        equal(testStrings.webpageTrimmed, tree
            .getTopDownArray()
...
helpButton abstractButton`;
        ...
        // Assert
        equal(inheritanceTree.toString(), `abstractNodeType
...
d`);
    ...
    testTree.getStackString = equal => {
        const tree = new TreeNode(`Thing
 color
  blue
  green`);
        // Act/assert
        equal(tree.getNode("Thing color green").getStackString(), `Thing
 color
  green`);
    };
...
Mammal Animal
 milk`);
    ...
    testTree.getGraphLoop = equal => {
        // Arrange
        const tree = new TreeNode(`Thing Animal
 color
Animal Thing
 dna`);
        // Act/Assert
...
use red programmingLanguages`;
        ...
        // Assert
        equal(expanded.toString(), `>reddit programming
...
chart`;
 ...
 pie`;
        ...
        // Arrange/Act/Assert
        equal(new TreeNode(`file foo.js
a = 2
b = 3
c = 4`)
            .nodeAtLine(0)
...
Animal Thing
 ...
 ab overridden`);
        // Act/Assert
        equal(tree._expandChildren(0, 1).toString(), `Thing
 color
 ab
Animal Thing
 color
 ...
 dna`);
        // Arrange
        const tree2 = new TreeNode(`Thing
 color
Animal Thing
 dna
Monkey Mammal
...
Mammal Animal
 milk`);
        // Act/Assert
        equal(tree2._expandChildren(0, 1).toString(), `Thing
 color
Animal Thing
 color
 dna
...
car non-existant`);
    ...
    testTree.expandedShouldAppendNonMaps = equal => {
        ...
        // Arrange
        const tree = new TreeNode(`constructors
 example
 example foobar`);
        // Act/Assert
...
`;
    ...
    testTree.copyTo = equal => {
        // Arrange
        const value = new TreeNode(`chart
 title Hello
chart2
 title 2`);
 ...
 title Hello`;
    ...
    testTree.copyToRegression = equal => {
        // Arrange
        const tree = new TreeNode(`>something
 class SomeClass
 css
  red
  green
 ...
 >div`;
        const migrateNode = (node) => {
            if (!node.getFirstWord().startsWith(">"))
                return true;
            if (node.length) {
...
q quantity`);
        ...
        expected.forEach((node) => node.remap(contractMap));
        // Assert
 ...
 age 10`);
    ...
    testTree.reverse = equal => {
        ...
        equal(tree2
            .nodeAt(0)
            .nodeAt(0)
            .getFirstWord(), "age", "Expected reversed properties");
        equal(tree2
            .nodeAt(1)
            .nodeAt(0)
            .getFirstWord(), "name", "Expected unchanged properties");
    };
    testTree.set = equal => {
        // Arrange
        const tree = new TreeNode("hello world");
...
bob
 age 10`);
        ...
        equal(tree.shift().toString(), `john
 age 5`, "expected correct string returned");
        equal(tree.length, 2);
...
\rthree`);
    ...
    testTree.traverse = equal => {
        // Arrange
        const traversal = new TreeNode(`0
 01
 02
  020
  021
...
2`);
        ...
        // Arrange
        const wikipediaBinaryTree = new TreeNode(`f
 b
  a
  d
   c
...
b
 a
c`).getFiltered((node) => node.getFirstWord() === "a").length, 3);
    };
...
b
 a
c`).deleteDuplicates().length, 3);
    };
    testTree.fromShape = equal => {
        // AAA
        equal(TreeNode.fromShape([2, 2]).toString(), `0
 0
 1
1
 0
 1`);
    ...
    testTree.toOutline = equal => {
        // Arrange
        const treeNode = new TreeNode(`hello
 world`);
        // Act/assert
        equal(treeNode.toOutline(), `└hello
 └world
`);
        equal(treeNode.toMappedOutline((node) => "o"), `└o
 └o
`);
    };
 ...
 d`);
    ...
    testTree.setTests = equal => {
        ...
        equal(base
            ...
            .getWordsAsSet()
            .has("bam"), true, "word should be appended");
    };
    ...
    testTree.queryMethods = equal => {
        ...
        equal(tree
            ...
            .sortBy("species")
            .nodeAt(0)
            .get("species"), "virginica", "last and first work");
    };
}
