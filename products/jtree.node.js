...
class TreeUtils {
    ...
    static joinArraysOn(joinOn, arrays, columns) {
        ...
        let index = 0;
        if (!columns)
            columns = arrays.map(arr => Object.keys(arr[0]));
...
class TreeNode extends AbstractNode {
    ...
    _getUid() {
        if (!this._uid)
            this._uid = TreeNode._makeUniqueId();
        return this._uid;
    }
...
class TreeNotationCodeMirrorMode {
    ...
    _getCellStyle(lineIndex, cellIndex) {
        ...
        if (!program.getCellHighlightScopeAtPosition)
            console.log(program);
...
jtreeNode.getProgramConstructor = (grammarPath) => {
    if (!fs.existsSync(grammarPath))
        throw new Error(`Grammar file does not exist: ${grammarPath}`);
