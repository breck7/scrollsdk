...
testTree.constructorTests = equal => {
	...
	const tree6 = new TreeNode({
		foobar: new TreeNode(testStrings.helloWorld)
	})
...
testTree.getWordProperties = equal => {
	// Arrange
	const tree = new TreeNode(aTestCase)
...
testTree.forEach = equal => {
	...
	// Arrange
	const value2 = new TreeNode(aTestCase)
...
testTree.first = equal => {
	...
	// Assert
	equal(value2.nodeAt(0).asString, testStrings.helloWorld)
}
...
testTree.getPathVector = equal => {
	...
	// Assert
	equal(parent.getPathVector().join(TN_WORD_BREAK_SYMBOL), indexPath.join(TN_WORD_BREAK_SYMBOL))
	equal(child.getPathVector().join(TN_WORD_BREAK_SYMBOL), "5	0	4	0	0	0")
	...
	// Assert
	equal(newNamePath.join(TN_WORD_BREAK_SYMBOL), namePath)
}
...
deep
	...
	equal(tree.has("deep	test"), true, "2 levels")
	equal(tree.has("deep	test	2"), true, "3 levels")
...
testTree.last = equal => {
	...
	// Arrange
	const value2 = new TreeNode(testStrings.hiMom)
	// Assert
...
testTree.set = equal => {
	...
	// TEST ORDERING
	// Arrange
	const tree4 = new TreeNode(testStrings.helloWorld)
	// Act
	...
	tree5.touchNode("yo").setChildren(new TreeNode(testStrings.helloWorld))
	// Assert
...
testTree.asyncUndoRedo = async equal => {
	// Arrange
	const node = new TreeNode(testStrings.helloWorld)
